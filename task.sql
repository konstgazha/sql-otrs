...
(
	...
	FROM
	(
		SELECT thid, tn, tcreatetime, thpendingautoclose, thclose, artbody, sername, name, diff, create_time1, create_time2, tsname, qname
		FROM
		(
						...
						(
							...
							(
										...
										(
											...
											FROM
											    (
											        ...
											        WHERE
											            th.create_time > '2018-04-01 00:00:00'
											...
											RIGHT JOIN
											    (
											        ...
											        FROM ticket t
											        WHERE
											            t.create_time > '2018-04-01 00:00:00'
											    ) s2
											ON s2.tid = s1.thid
										) s3
										LEFT JOIN
								...
								(
									...
									FROM article art
									WHERE
										art.create_time > '2018-04-01 00:00:00'
					...
					(
						...
						(
							...
							(
								...
								AND th.create_time > '2018-04-01 00:00:00'
								GROUP BY th.ticket_id
							) s15
							RIGHT JOIN
							(
	...
	LEFT JOIN
	(
		SELECT ticket_id, MAX(a_body) a_body
		FROM article a
		#LEFT JOIN ticket t ON a.ticket_id = t.id
	   WHERE a.create_time > '2018-04-01 00:00:00'
		AND a.a_subject = 'Заметка'
		GROUP BY a.id
	) s26
	ON s25.thid = s26.ticket_id
) s27
LEFT JOIN
(
	SELECT ticket_id, name sender, min(history_type_id)
	FROM ticket_history th
	WHERE history_type_id in (4, 24)
	AND create_time > '2018-04-01 00:00:00'
	GROUP BY th.id
) s28
ON s27.thid = s28.ticket_id